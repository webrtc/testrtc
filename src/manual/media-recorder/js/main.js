...
function getUserMedia() {
  var enableAudio = isTrue(getSelectedValue('enableAudio'));
  var enableVideo = isTrue(getSelectedValue('enableVideo'));
...
function buildRecorderConfig() {
  var options = {};
  var stream = getSelectedValue('recordedDtream');
  var audioBitrate = getSelectedValue('audioBitrate');
  var videoCodec = getSelectedValue('videoCodec');
  var videoBitrate = getSelectedValue('videoBitrate');
  var overallBitrate = getSelectedValue('overallAvBitrate');
  if (stream == 'local') {
...
function createRecorder() {
  ...
  var ignoreMutedMedia = isTrue(getSelectedValue('ignoreMuted'));
...
function startRecorder() {
  ...
  timeDataReceived = [];
  eventCounter = {};
  var timeSlice = getSelectedValue('timeSlice');
  if (timeSlice) {
...
function computeAndDisplayEventCount(eventName, initialize) {
  ...
  document.getElementById(eventName + 'Count').value = eventCounter[eventName];
}

// This function displays the media recoder's state.
function displayRecorderState() {
  document.getElementById('recorderState').value = theRecorder.state;
}
...
function playback() {
  ...
  playbackVideo.src = window.URL.createObjectURL(videoBlob);
}
